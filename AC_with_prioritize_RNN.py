...
class ActorCritic:
    def __init__(self, env, sess, is_endless = False, load_model = False):
        ...
        self.actor_state_input, self.actor_state_input_2, self.actor_task_input, self.actor_model = self.create_actor_model(self.actor_opt)
        self.temp_actor_state_input, self.temp_actor_state_input_2, self.temp_actor_task_input, self.temp_actor_model = self.create_actor_model(
            self.temp_actor_opt)
        _, _, _, self.target_actor_model = self.create_actor_model(self.target_actor_opt)
        #actor_model_weights = self.actor_model.trainable_weights

        self.critic_state_input,  self.critic_state_input_2,\
        self.critic_model = self.create_critic_model(self.critic_opt)
        self.temp_critic_state_input, self.temp_critic_state_input_2,\
        self.temp_critic_model = self.create_critic_model(self.temp_critic_opt)
        _, _, self.target_critic_model = self.create_critic_model(self.target_critic_opt)
    ...
    def create_actor_model(self, opt):
        ...
        if(not self.load_model):
            state_input = Input( shape = (bufferLength,1) )
            state_2_input = Input( shape = (bufferLength,1) )
            memory_layer = LSTM( 64 )(state_input)
            memory_2_layer = LSTM( 64 )( state_2_input)
            memory_output = Concatenate()([memory_layer, memory_2_layer])
            task_input = Input(shape=self.env.task_space.shape)
            memory_output_with_task = Concatenate()([memory_output, task_input])
            h1 = Dense(256, activation='relu')(memory_output_with_task)
            output = Dense(self.env.action_space.shape[0], activation='softmax')(h1)

            model = Model(input=[state_input, state_2_input, task_input], output=output)
            # adam = Adam(lr=0.001)
            model.compile(loss="mse", optimizer=opt)
            return model.input[0], model.input[1], model.input[2], model
        else:
    ...
    def create_critic_model(self,opt):
        if (not self.load_model):
            ...
            memory_2_layer = LSTM(64)(state_2_input)
            memory_output = Concatenate()([memory_layer, memory_2_layer])
            h2 = Dense(256, activation='relu')(memory_output)
            output = Dense(1, activation='linear')(h2)
            model = Model(input=[ state_input, state_2_input ], output=output)
            model.compile(loss="mse", optimizer=opt)
            return state_input, state_2_input, model
        else:
            model = load_model('model_c_rnn')
            model.compile(loss="mse", optimizer=opt)
    ...
    def init_gradient(self):
        ...
        self.entropy = tf.negative( tf.reduce_sum( tf.multiply( tf.log(self.actor_model.output), self.actor_model.output ) ) )
        self.actor_entropy_grads = tf.gradients( tf.negative(self.entropy) * self.beta, self.actor_model.trainable_weights )

        for i, g in enumerate(self.actor_grads):
            if g is not None:
                self.actor_grads[i] = tf.clip_by_norm(g, self.gradient_clip_thres)

        for i, g in enumerate(self.actor_entropy_grads):
            if g is not None:
                self.actor_entropy_grads[i] = tf.clip_by_norm(g, self.gradient_clip_thres)
        ...
        actor_entropy_grads = zip(self.actor_entropy_grads, self.temp_actor_model.trainable_weights)
        ...
        self.apply_entropy = self.temp_actor_opt.apply_gradients(actor_entropy_grads)

    def train(self):
        if len(self.memory)<self.batchSize : return 0
        self.trainCounter+=1
        ...
        '''
        cur_states = [ m[0] for m in self.memory ]
        
        for state in cur_states:
            v_curs = self.critic_model.predict( state )
        '''
        totalDelta = 0
        for i in range(self.batchSize):
            #ind = random.randrange(len(self.memory))
            ind = np.random.choice( len(self.priority), p = self.priority/self.priority.sum() )
            (cur_state, action_ind, reward, new_state, done, I, task) = self.memory[ind]
            if (not done):
                v_future = self.target_critic_model.predict( [new_state[0], new_state[1]] )
            else:
                v_future = np.array([[0]])
            v_current = self.critic_model.predict( [cur_state[0], cur_state[1] ])
            ...
            totalDelta += delta
            self.priority[ind] = np.abs(delta)
            ...
            self.sess.run(self.train_critic, feed_dict={
                self.critic_state_input: cur_state[0],
                self.critic_state_input_2: cur_state[1],
                self.delta: delta[0],
                self.I: I,
            ...
            self.sess.run(self.train_actor, feed_dict={
                    self.actor_state_input: cur_state[0],
                    self.actor_state_input_2: cur_state[1],
            ...
            '''
            self.sess.run(self.apply_entropy, feed_dict={
    ...
    def calc_td_error(self, cur_state, new_state, reward):
        cur_v = self.critic_model.predict( [cur_state[0], cur_state[1]] )
        next_v = self.target_critic_model.predict( [new_state[0], new_state[1]]  )
        delta = reward + self.gamma*next_v - cur_v
        return delta
    ...
    def remember(self, cur_state, action_ind, reward, new_state, done, I, task):
        ...
        self.priority = np.append(self.priority, np.abs(self.calc_td_error(cur_state, new_state, reward)) )
    ...
    def act(self, cur_state, task):
        ...
        '''
        return self.target_actor_model.predict( [cur_state[0], cur_state[1], task] )
...
def state_to_input(state):
    ...
    return np.array(res[0]).reshape( 1, len(res[1]), 1 ), np.array(res[1]).reshape( 1, len(res[1]), 1 )
...
def main():
    ...
    for round in range(20000):
        if( (round+1)%500 == 0):
            actor_critic.critic_model.save('model_c_rnn')
            actor_critic.actor_model.save('model_a_rnn')
        print(round)
        total_t = 0
        total_correct = 0
        total_incorrect = 0
        ...
        for ind in range(30):
            ...
            if (reward == 0): break
        f = open('delta.txt', 'a+')
        f.write(str(total_td)+'\n')
        f.close()
        print('total td: ', total_td)
        f = open('res.txt','a+')
